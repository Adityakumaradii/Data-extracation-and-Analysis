...
# Load positive and negative words (assuming files are in the current directory)
# Expanded list of positive words
positive_words_list = ["a+",
"abound",
"abounds",
"abundance",
"abundant",
...
# Create and write to positive-words.txt
with open('positive-words.txt', 'w', encoding="utf-8") as file:
    for word in positive_words_list:
        file.write(word + "\n")

# Create and write to negative-words.txt
with open('negative-words.txt', 'w', encoding="utf-8") as file:
...
def compute_variables(text):
    ...
    pos_score = sum(1 for word in filtered_words if word.lower() in positive_words_list)
    neg_score = sum(1 for word in filtered_words if word.lower() in negative_words_list)
    polarity_score = (pos_score - neg_score) / ((pos_score + neg_score) + 0.000001)
    subjectivity_score = (pos_score + neg_score) / (word_count + 0.000001)
    avg_sentence_len = avg_sentence_length(text)
    percentage_complex = percentage_complex_words(filtered_words)
    fog = fog_index(text)
    avg_words_per_sent = avg_words_per_sentence(text)
    complex_word_count = sum(1 for word in filtered_words if is_complex(word))
    syll_per_word = syllables_per_word(filtered_words)
    personal_pronouns = personal_pronouns_count(filtered_words)
...
def fog_index(text):
    return 0.4 * (avg_sentence_length(text) + percentage_complex_words(word_tokenize(text)))
...
# Function to compute syllable count of a word
...
def is_complex(word):
    return syllable_count_word(word) > 2

# Function to compute personal pronouns count
def personal_pronouns_count(words):
    personal_pronouns = ['i', 'we', 'my', 'ours', 'us']
    return sum(1 for word in words if word.lower() in personal_pronouns)

# Function to process each text file and compute variables
...
# Main function
def main():
    directory = r"C:/Users/adity/Downloads/textfiles"
    output_file = r"C:/Users/adity/downloads/output.xlsx"

    data = process_files_in_directory(directory)
